...
import { UploadCloud, Sparkles, FolderOpen, ArrowRight } from 'lucide-react'
...
export default function HomeView() {
  const { workbooks, loadDemoWorkbook, openWorkbook } = useWorkbook()
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto">
      <h1 className="font-display text-2xl font-semibold text-ink mb-1">Welcome back{firstName ? `, ${firstName}` : ''}</h1>
      <p className="text-sm text-ink-muted mb-8">Turn any spreadsheet into a dashboard, instantly.</p>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-10">
        ...
        >
          ...
          <div>
            <p className="font-medium text-ink mb-0.5">Upload workbook</p>
            <p className="text-xs text-ink-muted">xlsx, xls, csv, or ods</p>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <p className="font-medium text-ink mb-0.5">Try demo data</p>
            <p className="text-xs text-ink-muted">Explore a sample sales workbook</p>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <p className="font-medium text-ink mb-0.5">Open workbooks</p>
            <p className="text-xs text-ink-muted">{workbooks.length} saved</p>
          </div>
        </button>
      </div>

      <div>
        <h2 className="text-sm font-medium text-ink-muted mb-3">Recent workbooks</h2>
        {workbooks.length === 0 ? (
          <EmptyState
            ...
            {workbooks.slice(0, 5).map((wb) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium text-ink truncate">{wb.name}</p>
